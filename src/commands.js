...
exports.handleCommands = (bot, gameManager) => {
  // Старт игры
  bot.start(ctx => gameManager.initLobby(ctx.chat.id));
  ...
  bot.on('text', async ctx => {
    ...
    const ok = await gameManager.recordAnswer(ctx.from.id, ctx.message.text);
    if (ok) await ctx.reply('✅ Ответ принят! Жди голосования.');
  });
};
